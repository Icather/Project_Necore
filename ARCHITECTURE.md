# Project_Necore — 架构设计与技术指南

> 📖 本文档详细记录了项目的架构设计、技术选型与实现细节。面向开发者和贡献者。
> 
> 👉 项目概览与快速上手请移步 [README.md](README.md)
...
### 3.10 局域网同步 (LAN Sync)　_(2026-05-17)_
同一 WiFi 下两台手机通过 UDP 广播自动发现 + 点击确认配对，实现应用数据的端到端加密同步。核心架构：
1. **发现机制**：发送方通过 NanoHTTPD 启动临时 HTTP 服务器并以 UDP 广播（端口 19527）公布设备信息，接收方自动扫描发现。
2. **ECDH + AES-256-GCM 加密**：连接时双方进行椭圆曲线 Diffie-Hellman 密钥协商，派生 AES-256 密钥加密全部传输数据。全程使用 Android 标准库 API，零第三方密码学依赖。
3. **SAS 短认证码**：从共享密钥独立派生 4 位验证码，双方屏幕同时显示供用户肉眼核对，防止中间人攻击（蓝牙 Secure Simple Pairing 同款机制）。
4. **数据分类与模式**：聊天记录（Conversation + Message）和 API 配置可分开同步，支持覆盖同步和增量同步两种模式，完全复用 `SettingsRepository` 已有的 JSON 导入导出逻辑。
