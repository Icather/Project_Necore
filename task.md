...
## 分类 C：[难度 ⭐️⭐️⭐️ | 可行性 极高] 视觉重塑与极简交互 (DeepSeek / 豆包 官方风)
*(目标：摒弃原生 Material 粗糙感，1:1 致敬顶级商业 AI 助手的无边界界面)*
- `[x]` **C1. 无背景沉浸式 AI 发言**: 移除 Assistant 气泡背景，文本直贴底色，左侧加入 `✨` 图标标识，打造极简阅读体验。
- `[x]` **C2. 贴底纯净输入坞**: 取消悬浮卡片，输入区域死死贴底。重构内部组件，采用淡灰色胶囊输入框，并将附件、发送按钮紧凑收纳。
- `[x]` **C3. 极客开关微缩化**: 将“模型切换”和“思考模式”开关缩小为胶囊按钮，紧贴文本框上方，彻底释放屏幕空间。
- `[x]` **C4. 极简呼吸顶部栏**: 移除色块，居中显示模型名称，最大化弱化非必要 UI 元素的视觉比重。

## 分类 D：[难度 ⭐️⭐️⭐️ | 可行性 具挑战] 物理极限压榨与远景引擎 (Limits & Soul Engine)
